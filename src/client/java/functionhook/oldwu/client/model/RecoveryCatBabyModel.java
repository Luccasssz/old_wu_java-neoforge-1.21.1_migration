...
/**
 * 回血状态幼年模型（Blockbench 导出后适配 26.2 渲染 API，32×32）。
 */
public class RecoveryCatBabyModel extends OldWuCatModel {
	public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(Old_Wu_java.id("cat_recovery_baby"), "main");
	...
	public RecoveryCatBabyModel(ModelPart root) {
		super(root);
	...
	public void setupAnim(Cat cat, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		this.head.xRot += Mth.sin(age * 0.15F) * 0.02F;
		this.tail1.zRot += Mth.sin(age * 0.3F) * 0.06F;
